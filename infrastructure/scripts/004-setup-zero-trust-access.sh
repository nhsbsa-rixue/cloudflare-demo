...
# - ACCESS_APP_LOGO_URL        (optional HTTPS logo URL for App Launcher surfaces)
...
ACCESS_APP_LOGO_URL="${ACCESS_APP_LOGO_URL:-}"
ACCESS_APP_LAUNCHER_VISIBLE="${ACCESS_APP_LAUNCHER_VISIBLE:-false}"
...
export ACCESS_APP_NAME ACCESS_APP_LOGO_URL ACCESS_APP_LAUNCHER_VISIBLE ACCESS_POLICY_NAME ACCESS_SESSION_DURATION ACCESS_AUTO_REDIRECT
...
const payload = {
  ...
  app_launcher_visible: process.env.ACCESS_APP_LAUNCHER_VISIBLE === 'true'
};

if (process.env.ACCESS_APP_LOGO_URL) {
  payload.logo_url = process.env.ACCESS_APP_LOGO_URL;
}
