...
export class CasesHelper {
  ...
  async insertCase(caseData: NewCase): Promise<Result<Case>> {
    try {
      ...
      return Err(
        new DatabaseError("Failed to insert case", "INSERT_CASE_ERROR", error)
      );
    }
  }
  ...
  async getCase(id: string): Promise<Result<Case | null>> {
    try {
      ...
      return Err(
        new DatabaseError("Failed to fetch case", "GET_CASE_ERROR", error)
      );
    }
  }
  ...
  async getCasesByUser(
    ...
    try {
      ...
      return Err(
        new DatabaseError(
          "Failed to fetch cases by user",
          "GET_CASES_BY_USER_ERROR",
          error
        )
      );
    }
  ...
  async updateCase(
    ...
    try {
      ...
      return Err(
        new DatabaseError("Failed to update case", "UPDATE_CASE_ERROR", error)
      );
    }
  }
  ...
  async deleteCase(id: string): Promise<Result<boolean>> {
    try {
      ...
      return Err(
        new DatabaseError("Failed to delete case", "DELETE_CASE_ERROR", error)
      );
    }
  }
    ...
    try {
      let query = this.db
        ...
        .where(
          and(
            eq(cases.userId, userId),
            eq(cases.status, status as "draft" | "active" | "completed" | "archived")
          )
        );
      ...
      return Err(
        new DatabaseError(
          "Failed to fetch cases by user and status",
          "GET_CASES_BY_USER_AND_STATUS_ERROR",
          error
        )
      );
    }
